...
<img src="/published.png" alt="Example of Published Package"/>
...
You can't create a `Transaction Context` manually. The system passes that in when someone calls your code.
...
```move [sui.move]
nums.push_back(4)
let last_value = nums.pop_back()
```

This is just just syntactic sugar and is functionally equivalent to explicitly calling the library function.
